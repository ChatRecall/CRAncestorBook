...
PHASE_STYLE_PREREQS = StepPrereqs(
    required_dirs=("Chapters",),
    required_globs=(
        GlobRequirement(
            pattern=chapter_glob(CODE_DUP_RESOLVE),
            missing_msg=(
                f"Missing Phase Style inputs: no {chapter_glob(CODE_DUP_RESOLVE)} files found.\n"
                "Run Duplicate Resolution first, or confirm your Duplicate Resolution output path."
            ),
        ),
    ),
)
...
# Step Style Polish (one prompt per chapter)
STEPS: list[StepDefinition] = [
    StepDefinition(
        code=CODE_STYLE_POLISH,
        name="Style Polish",
        prompt_name="chapter_style_polish",
        output_relpath_pattern=chapter_output(CODE_STYLE_POLISH),
        payload_relpath_pattern=chapter_payload(CODE_STYLE_POLISH),
    )
]

def _phase_dedup_final_input_path(ctx: BookContext, chapter_n: int) -> Path:
    root = ctx.root
    return chapter_path(root, chapter_n, CODE_DUP_RESOLVE)


def inputs_for_step(step_code: str, ctx: BookContext, chapter_n: int) -> list[Path]:
    if step_code == CODE_STYLE_POLISH:
        return [_phase_dedup_final_input_path(ctx, chapter_n)]
...
def values_for_step(ctx: BookContext, chapter_n: int, step: StepDefinition) -> dict[str, Any]:
    ...
    # - %% chapter_text %%
    return {
        "chapter_text": _phase_dedup_final_input_path(ctx, chapter_n),
    }

def run_phase_style(ctx: BookContext, *, chapters: list[int], prompt_lib: Any) -> None:
    ok, msg = verify_step_prereqs(ctx.root, PHASE_STYLE_PREREQS)
    if not ok:
        error(msg)
        return
    ...
    for chapter_n in chapters:
        ...
        ai_model = get_model_substep(CODE_STYLE_POLISH)
        ...
        if not input_path.exists():
            error(f"Missing input for style_polish: {input_path}")
            error("Update _final_chapter_input_path() to point at your dedup-resolution output.")
            continue
        ...
        if not ok:
            error(f"Stopping Style Polish for remaining chapters after failure in {step.name}")
            break

    complete("Style Polish complete.")
